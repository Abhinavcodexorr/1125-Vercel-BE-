...
/** Remove the cart owner's earlier unpaid hold so re-creating (e.g. after editing) replaces it instead of stacking. */
const removePreviousPendingBookingsForCart = async (cartId) => {
    if (!cartId) return;
...
const createRoomBooking = async (req, res) => {
    try {
        ...
        if (cartId) {
            ...
            await removePreviousPendingBookingsForCart(cartId);
